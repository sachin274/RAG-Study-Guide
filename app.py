...
if sys.platform == 'win32':
    ...
    # The default Windows console codepage (cp1252) can't encode the emoji
    # used in print() statements across the pipeline (e.g. checkmarks in
    # pdf_extracter.py), which crashes those prints and masks real errors.
...
def create_study_guide_markdown_with_gemini(gemini_content, metadata, output_path):
    ...
    # Single top-level title (Gemini content starts at ## so it nests under this cleanly)
    topics_str = metadata.get('topics', 'N/A')
    content.append(f"# Study Guide: {topics_str}\n\n")

    # Metadata as a compact table instead of a competing heading block
    content.append("| | |\n")
    content.append("|---|---|\n")
    content.append(f"| **Source File** | {metadata.get('source_file', 'N/A')} |\n")
    content.append(f"| **Topics Requested** | {topics_str} |\n")
    content.append(f"| **Generated** | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} |\n")
    content.append(f"| **Sections Analyzed** | {metadata.get('relevant_count', 'N/A')} of {metadata.get('total_chunks', 'N/A')} chunks |\n")
    content.append(f"| **Generated With** | Google Gemini Pro |\n")
    content.append("\n---\n\n")

    # Main Study Content (from Gemini) - already formatted starting at ##
